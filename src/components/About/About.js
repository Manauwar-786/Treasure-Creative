...
<Link href="/"><IoIosArrowRoundBack className="h-15 w-15 pl-4 pt-5 md:hidden animate-pulse" /></Link>
     ...
     data-aos-duration="3000"
      ...
      <section className="md:px-[120px] md:py-10 py-5 px-4">
        {hopeContent.map((item, index) => (
          <div key={index} className="md:px-10 md:py-5 px-4 py-5">
          ...
          {chooseUsContent.map((item, index) => (
            <div key={index} className="border rounded px-4 py-5">
              <img
                ...
                src={item.image}
              ></img>
              <h2 className="text-xl font-bold pt-2 md:py-5">{item.heading}</h2>
              <p className="font-mono">{item.para}</p>
            </div>
          ))}
        </div>
